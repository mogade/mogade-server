...
describe Rank, :save do
  it "saves a daily rank" do
    ...
    key = "#{leaderboard.id}_daily_#{leaderboard.daily_start.strftime("%Y%m%d%H")}"
  ...
  it "saves a weekly rank" do
    ...
    key = "#{leaderboard.id}_weekly_#{leaderboard.weekly_start.strftime("%Y%m%d%H")}"
